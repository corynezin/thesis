...
for file_name in os.listdir(root_dir):
        ...
        with g.as_default():
            ...
            with tf.Session() as sess:
                ...
                if rnn_sentiment == rv.sentiment and val_found:
                    ...
                    np.save('./gaws_testing/ix/'+file_name[-4:]+'.npy',ix)
                    np.save('./gaws_testing/jx/'+file_name[-4:]+'.npy',jx)
                    minm = min(m+1,minm)
                    val_found = True
                    R = m - 1
                if L > R:
    ...
    t.append(t1-t0)
    np.save('./gaws_testing/ii/' + file_name[:-4] + '.npy',ii)
    np.save('./gaws_testing/jj/' + file_name[:-4] + '.npy',jj)
    np.save('./gaws_testing/pp/' + file_name[:-4] + '.npy',pp)
    np.save('./gaws_testing/t/'+file_name[:-4] + '.npy',np.array(t))
    if sum(c) % 50 == 0:
        print('histogram:',c)
        print('fails:',f)
