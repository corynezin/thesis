        ...
        with g.as_default():
            ...
            with tf.Session() as sess:
                ...
                for ins_location in range(per):
                    print(ins_location)
                    _,p,_,im,rep_vec = r.infer_swap(sess,rv,ins_location,divs,top_k)
                    p = p[:,0]
                    ...
                    if rv.sentiment == 'pos':
                        for i in range(divs):
                            if min_arr[i] == np.amin(p,axis=0)[i]:
                                ii[i] = rep_vec[np.argmin(p[:,0])]
                                jj[i] = ins_location + per*i
                                print(min_arr)
                    else:
                        for i in range(divs):
                            if max_arr[i] == np.amax(p,axis=0)[i]:
                                ii[i] = rep_vec[np.argmax(p[:,0])]
                                jj[i] = ins_location + per*i
                                print(max_arr)
                np.save('./minmax/'+str(divs)+'/ii/'+name,np.array(ii))
                np.save('./minmax/'+str(divs)+'/jj/'+name,np.array(jj))
