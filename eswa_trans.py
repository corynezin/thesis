...
for file_name in os.listdir(root_dir):
        ...
        with g.as_default():
            ...
            with tf.Session() as sess:
                ...
                if rnn_sentiment == rv.sentiment and val_found:
                    ...
                    np.save('./wa_tran_testing/ix/'+file_name[:-4]+'.npy',ix)
                    np.save('./wa_tran_testing/jx/'+file_name[:-4]+'.npy',jx)
                    minm = min(m+1,minm)
                    val_found = True
                    R = m - 1
                if L > R:
    ...
    t.append(t1-t0)
    np.save('./wa_tran_testing/ii/' + file_name[:-4] + '.npy',ii)
    np.save('./wa_tran_testing/jj/' + file_name[:-4] + '.npy',jj)
    np.save('./wa_tran_testing/pp/' + file_name[:-4] + '.npy',pp)
    np.save('./wa_tran_testing/t/'+file_name[:-4] + '.npy',np.array(t))
    np.save('./wa_tran_testing/c/'+file_name[:-4] + '.npy',np.array(c))
    if sum(c) % 50 == 0:
        print('histogram:',c)
        print('Fails:',f)
